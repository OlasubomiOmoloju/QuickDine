...
const seedData = async ()=>{
    try {
        ...
        const adminPassword = await brcypt.hash("admin123", salt);
        const userPassword = await brcypt.hash("user123", salt);
        const ownerPassword = await brcypt.hash("owner123", salt);
